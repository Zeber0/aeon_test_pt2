...
class User {
    ...
    public static function user_return_info($user_id) {
    ...
    public static function users_list($d = []) {
        ...
        // where
        $where = [];
        if ($search) $where[] = "first_name LIKE '%".$search."%' OR last_name LIKE '%".$search."%' OR phone LIKE '%".$search."%' OR email LIKE '%".$search."%'";
        ...
        while ($row = DB::fetch_row($q)) {
            $items[] = [
                ...
                'phone' => $row['phone'],
                'email' => $row['email'],
                'last_login' => date('Y/m/d', strtotime($row['last_login']))
            ];
        }
    ...
    public static function user_edit_window($d = []) {
        $user_id = isset($d['user_id']) && is_numeric($d['user_id']) ? $d['user_id'] : 0;
        HTML::assign('user', User::user_return_info($user_id));
    ...
    public static function user_edit_update($d = []) {
        ...
        $plot_id = isset($d['plot_id']) ? trim($d['plot_id']) : '';
